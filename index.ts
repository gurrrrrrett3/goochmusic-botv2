...
Client.on("messageCreate", async (message) => {
  ...
  if (enabledCommands[command] == false) {
    message.reply(`${emoji.failure} Sorry ${message.author.username}, that command is disabled.`);
    return;
  }
...
function annoy(text: string) {
  ...
  for (var i = 0; i < text.length; i++) {
    array.push(Math.random() > 0.5 ? text.charAt(i).toLowerCase() : text.charAt(i).toUpperCase());
  }
