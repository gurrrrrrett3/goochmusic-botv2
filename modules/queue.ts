...
export default class Queue {
    ...
    public async generateQueueImage(message: Message) {
        ...
        const text = `${this.q[0].metadata.videoData.title} | ${formatTime(this.q[0].metadata.videoData.length)}`
        ...
        for (let i = 1; i < (this.q.length > 8 ? 8 : this.q.length); i ++) {
            ...
            await this.drawUserIcon(ctx, 5, currentLine, this.q[i].metadata.reqestUser, message)

            //Draw the song name

            const text = `${this.q[i].metadata.videoData.title} | ${formatTime(this.q[i].metadata.videoData.length)}`
            ctx.font = this.applyText(canvas, text)
    ...
    private async drawUserIcon(ctx: CanvasRenderingContext2D, x: number, y: number, user: User, message: Message) {
